...
BEGIN
	...
	BEGIN TRY
		...
		SELECT
			...
			CAST(
				CAST(LEAD(prd_start_dt) OVER(PARTITION BY prd_key ORDER BY prd_start_dt) AS DATETIME) - 1
		...
		SELECT 
			...
			END AS bdate,
			CASE
				WHEN UPPER(TRIM(gen)) LIKE 'FEMALE%' OR UPPER(TRIM(gen)) LIKE 'F%' THEN 'Female'
